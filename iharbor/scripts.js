...
let lastSeenDownpaymentByOrderId = {}; // used to detect downpayment changes for the "updated" indicator
...
function startMyOrdersListener() {
  ...
  unsubscribeMyOrders = onSnapshot(q, snapshot => {
    ...
    orders.forEach(o => {
      ...
      const prevDownpayment = lastSeenDownpaymentByOrderId[o._id];
      const currDownpayment = o.downpayment || 0;
      if (prevDownpayment !== undefined && prevDownpayment !== currDownpayment) {
        hasNewUpdate = true;
        changeMessages.push('Downpayment recorded on your order');
      }
      lastSeenDownpaymentByOrderId[o._id] = currDownpayment;
    });
...
function renderMyOrders(orders) {
  ...
  orders.forEach(order => {
    ...
    const balanceDue  = (order.total || 0) - downpayment;
    ...
    li.innerHTML = `
      ...
      ${downpayment > 0 ? `
      <div class="order-card-payment" style="font-size:.82rem;color:#2c3e50;margin-top:4px;">
        Downpayment${order.paymentMethod ? ` (${order.paymentMethod})` : ''}: ₱${downpayment.toFixed(2)}
        <br><strong style="color:#e74c3c;">Balance Due: ₱${balanceDue.toFixed(2)}</strong>
      </div>` : ''}`;
    list.appendChild(li);
  });
      ...
      div.innerHTML = `
        ...
        <div class="product-footer">
          ...
          <div class="product-actions">
            ...
            <button type="button" class="addons-btn" data-name="${escHtml(p.name)}" data-price="${p.price}">🎀 Add-ons</button>
          </div>
        </div>`;
