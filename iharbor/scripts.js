...
async function openAddonsModal(name, price, color) {
  _addonsForProduct = { name, basePrice: parseFloat(price), color: color || '' };
...
function addonsModalSkip() {
  if (!_addonsForProduct) return;
  const { name, basePrice, color } = _addonsForProduct;
  closeAddonsModal();
  openQtyNotesModal(name, basePrice, color ? `Color: ${color}` : '');
}
  ...
  try {
    ...
    docs.forEach(d => {
      ...
      div.querySelector('.add-to-cart').addEventListener('click', function () {
        ...
        openQtyNotesModal(this.dataset.name, this.dataset.price, colorNote);
      });
      ...
      div.querySelector('.addons-btn').addEventListener('click', function () {
        ...
        openAddonsModal(this.dataset.name, this.dataset.price, div.dataset.selectedColor || '');
      });
