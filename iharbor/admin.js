...
window.exportOrdersCSV = async function () {
  ...
  const rows = orders.map(o => [
    ...
    o.deliveryOption,
    o.deliveryAddress || '',
    (o.items || []).map(i => `${i.name} x${i.quantity}${i.notes ? ` (${i.notes})` : ''}`).join('; '),
    (o.total || 0).toFixed(2),
    o.status
  ]);
...
function renderOrders(orders) {
  ...
  listEl.innerHTML = orders.map(order => {
    const itemsHtml = (order.items || [])
      .map(i => `<li>
        ${escapeHtml(i.name)} × ${i.quantity} — ₱${(i.price * i.quantity).toFixed(2)}
        ${i.notes ? `<br><span style="font-size:.8rem;color:#27ae60;font-style:italic;">📝 ${escapeHtml(i.notes)}</span>` : ''}
      </li>`)
      .join('');
