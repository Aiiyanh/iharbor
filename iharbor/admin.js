...
// ─── Products Lookup (fallback for orders placed before photo/description
// were saved on the order itself) ───────────────────────────────────────────────
// Keyed by product name so old order items (which only have name/price/qty/notes)
// can still show a photo and description, as long as a product with the same
// name still exists.
let _productsByName = {};
...
async function loadProductsLookup() {
  try {
    ...
    snap.forEach(d => {
      ...
      const image  = (colors.length && colors[0].image) ? colors[0].image : (p.image || '');
      _productsByName[p.name] = { image, description: p.description || '' };
    });
  } catch (e) {
    console.error('Could not load products for photo/description fallback:', e);
  }
  if (_lastOrders.length) renderOrders(_lastOrders);
}
...
// ─── Per-order Downpayment / Payment Method ────────────────────────────────────
// Admin records a downpayment amount and the channel it came through (GCash or
// GoTyme). Balance due = total - downpayment. Doesn't touch `total` itself —
// downpayment is tracked separately so the full order value stays intact.
window.saveOrderPayment = async function (docId, rawDownpayment, paymentMethod, total) {
  const statusEl    = document.getElementById(`dp-status-${docId}`);
  const downpayment = parseFloat(rawDownpayment);
  ...
  if (downpayment > total) {
    if (statusEl) { statusEl.textContent = 'Exceeds total.'; statusEl.style.color = '#e74c3c'; }
    return;
  }
...
<body>
  ...
  <table>
    ...
    ${order.downpayment ? `
    <tr>
      <td colspan="2">Downpayment${order.paymentMethod ? ` (${order.paymentMethod})` : ''}</td>
      <td style="text-align:right;">₱${order.downpayment.toFixed(2)}</td>
    </tr>
    <tr class="total-row">
      <td colspan="2">BALANCE DUE</td>
      <td style="text-align:right;">₱${((order.total || 0) - order.downpayment).toFixed(2)}</td>
    </tr>` : ''}
  </table>
...
// ─── Order Item Photo & Description Viewer ────────────────────────────────────
// Populated fresh each render; keyed by "orderId-itemIndex" since items don't
// have their own stable IDs.
let _itemLookup = {};

window.openOrderPhoto = function (itemKey) {
  const item = _itemLookup[itemKey];
  if (!item) return;

  const modal    = document.getElementById('admin-photo-modal');
  const mediaEl  = document.querySelector('.admin-photo-media');
  const imgEl    = document.getElementById('admin-photo-img');
  const descEl   = document.getElementById('admin-photo-description');

  document.getElementById('admin-photo-caption').textContent = item.name || '';
  ...
  if (item.description) {
    descEl.innerHTML = formatDescriptionAsBullets(item.description);
...
// Splits a product description into bullet points. Uses line breaks if the
// description already has them; otherwise falls back to splitting on
// sentence-ending punctuation so a single paragraph still reads as a list.
function formatDescriptionAsBullets(description) {
  ...
  parts = parts.map(stripLeadingEmoji).filter(Boolean);

  if (parts.length === 0) return '';
  return '<ul class="photo-desc-list">' + parts.map(p => `<li>${escapeHtml(p)}</li>`).join('') + '</ul>';
}

// Removes a leading emoji/symbol (and any following space) from a line, since
// our own bullet marker already provides that visual — without this, product
// descriptions that already start with an emoji (e.g. "🌹 9 roses") would show
// a broken-glyph box crammed right next to the bullet dot.
...
window.closeOrderPhoto = function () {
  document.getElementById('admin-photo-modal').classList.remove('show');
  document.getElementById('admin-photo-img').src = '';
  document.body.style.overflow = 'auto';
};

document.addEventListener('DOMContentLoaded', function () {
  const photoModal = document.getElementById('admin-photo-modal');
  if (photoModal) {
    photoModal.addEventListener('click', function (e) {
      if (e.target === this) closeOrderPhoto();
    });
  }
});
...
function renderOrders(orders) {
  ...
  _itemLookup = {};     // reset cache for photo/description viewer
  ...
  listEl.innerHTML = orders.map(order => {
    const itemsHtml = (order.items || [])
      .map((i, idx) => {
        ...
        const fallback = _productsByName[i.name] || {};
        const image       = i.image || fallback.image || '';
        const description = i.description || fallback.description || '';
        _itemLookup[itemKey] = { name: i.name, image, description };
        const hasMore = image || description;
        return `<li>
          ${image ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(i.name)}" class="order-item-thumb" onerror="this.style.display='none'" onclick="openOrderPhoto('${itemKey}')" />` : ''}
          <span class="order-item-text">
            ${escapeHtml(i.name)} × ${i.quantity} — ₱${(i.price * i.quantity).toFixed(2)}${i.notes ? `<br><span style="color:#27ae60;font-style:italic;">📝 ${escapeHtml(i.notes)}</span>` : ''}
            ${hasMore ? `<br><button type="button" class="item-see-more-btn" onclick="openOrderPhoto('${itemKey}')">See more</button>` : ''}
          </span>
        </li>`;
      })
      .join('');
    ...
    const balanceDue     = orderTotal - downpayment;
      ...
      <div class="order-card">
        ...
        <div class="order-card-body">
          ...
          <div class="payment-control">
            <label for="paymethod-${order._id}">Paid via</label>
            <select id="paymethod-${order._id}">
              <option value="GCash" ${order.paymentMethod === 'GCash' ? 'selected' : ''}>GCash</option>
              <option value="GoTyme" ${order.paymentMethod === 'GoTyme' ? 'selected' : ''}>GoTyme</option>
            </select>
            <label for="dp-input-${order._id}">Downpayment ₱</label>
            <input type="number" id="dp-input-${order._id}" min="0" max="${orderTotal}" step="0.01"
              value="${downpayment.toFixed(2)}" />
            <button class="delivery-fee-save-btn"
              onclick="saveOrderPayment('${order._id}', document.getElementById('dp-input-${order._id}').value, document.getElementById('paymethod-${order._id}').value, ${orderTotal})">Save</button>
            <span class="delivery-fee-status" id="dp-status-${order._id}"></span>
          </div>
          ${downpayment > 0 ? `<p class="order-downpayment">Downpayment (${escapeHtml(order.paymentMethod || '—')}): ₱${downpayment.toFixed(2)}</p>` : ''}
          <p class="order-balance"><strong>Balance Due: ₱${balanceDue.toFixed(2)}</strong></p>
        </div>
